...
void analysis_res_dist_eta_2()
{
  ...
  for (int i = 0; i < nbin; i++) {
  	  proj.Add(new TH1D(Form("%0.1f_eta_%0.1f", eta_bin[i], eta_bin[i+1]),"", 50, -0.075, 0.075));
	  fitArr.Add(new TF1(Form("fit%lu", i), "[0]/sqrt(2*TMath::Pi()*[2]^2)*exp(-0.5*((x-[1])/[2])**2) + [3]/sqrt(2*TMath::Pi()*[4]^2)*exp(-0.5*((x-[1])/[4])**2)", -0.075, 0.075));
  }
  ...
  for (int i = 35; i < 40; i++){
	...
	fit->SetParLimits(4, -0.075, 0.075);
	fit->SetParLimits(2, -0.075, 0.075);
	dist->Fit(fit, "rll");
